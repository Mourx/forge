...
public class ComputerUtilNN {
    ...
    public static int counterSpellRestriction(final Player ai, final SpellAbility sa) {
        ...
        if (unless != null && !unless.endsWith(">")) {
            ...
            final int usableManaSources = ComputerUtilMana.getAvailableManaSources(ComputerUtilNN.getOpponentFor(ai), true).size();
    ...
    public static CardCollection chooseSacrificeType(final Player ai, final String type, final SpellAbility ability, final Card target, final int amount) {
        ...
        while (count < amount) {
            Card prefCard = ComputerUtilNN.getCardPreference(ai, source, "SacCost", typeList);
            if (prefCard == null) {
    ...
    public static boolean castPermanentInMain1(final Player ai, final SpellAbility sa) {
        ...
        if (card.isCreature() && !card.hasKeyword(Keyword.DEFENDER)
                && (card.hasKeyword(Keyword.HASTE) || ComputerUtilNN.hasACardGivingHaste(ai, true) || sa.isDash())) {
            return true;
        }
        ...
        for (Card buffedcard : buffed) {
            ...
            if (card.isEquipment() && buffedcard.isCreature() && CombatUtil.canAttack(buffedcard, ComputerUtilNN.getOpponentFor(ai))) {
                return true;
            }
            if (card.isCreature()) {
        ...
        final CardCollectionView antibuffed = ComputerUtilNN.getOpponentFor(ai).getCardsIn(ZoneType.Battlefield);
    ...
    public static boolean shouldCastLessThanMax(final Player ai, final Card source) {
        ...
        if (source.getManaCost().countX() > 0) {
            ...
            Combat combat = new Combat(ComputerUtilNN.getOpponentFor(ai));
            CardCollectionView attackers = ComputerUtilNN.getOpponentFor(ai).getCreaturesInPlay();
            for (Card att : attackers) {
                if (ComputerUtilCombat.canAttackNextTurn(att, ai)) {
                    combat.addAttacker(att, ComputerUtilNN.getOpponentFor(att.getController()));
                }
            }
    ...
    public static int possibleNonCombatDamage(Player ai) {
        ...
        for (final Card c : all) {
            for (final SpellAbility sa : c.getSpellAbilities()) {
                ...
                final Player enemy = ComputerUtilNN.getOpponentFor(ai);
    ...
    public static List<GameObject> predictThreatenedObjects(final Player ai, final SpellAbility sa, boolean top) {
        ...
        for (SpellAbilityStackInstance si : game.getStack()) {
            ...
            if (sa == null || (sa != spell && sa != sub)) {
                Iterables.addAll(objects, ComputerUtilNN.predictThreatenedObjects(ai, sa, spell));
            }
            if (top) {
    ...
    private static Iterable<? extends GameObject> predictThreatenedObjects(final Player aiPlayer, final SpellAbility saviour,
        ...
        Iterables.addAll(threatened, ComputerUtilNN.predictThreatenedObjects(aiPlayer, saviour, topStack.getSubAbility()));
        return threatened;
    }
    ...
    public static boolean predictCreatureWillDieThisTurn(final Player ai, final Card creature, final SpellAbility excludeSa) {
        ...
        willDieFromSpell = !noStackCheck && ComputerUtilNN.predictThreatenedObjects(creature.getController(), excludeSa).contains(creature);
        ...
        if (aic.getBooleanProperty(AiProps.AVOID_TARGETING_CREATS_THAT_WILL_DIE)) {
            ...
            List<Card> willBeKilled = CardLists.filter(list, new Predicate<Card>() {
                ...
                public boolean apply(Card card) {
                    return card.isCreature() && ComputerUtilNN.predictCreatureWillDieThisTurn(ai, card, excludeSa);
                }
            });
        ...
        if (kindOfType.equals("Card")) {
            ...
            if (logic != null) {
                if (logic.equals("MostProminentOppControls")) {
                    ...
                    for (Card c : ComputerUtilNN.getOpponentFor(ai).getLandsInPlay()) {
    ...
    public static Object vote(Player ai, List<Object> options, SpellAbility sa, Multimap<Object, Player> votes) {
        ...
        case "GraceOrCondemnation":
            return ai.getCreaturesInPlay().size() > ComputerUtilNN.getOpponentFor(ai).getCreaturesInPlay().size() ? "Grace"
                    : "Condemnation";
    ...
    public static boolean aiLifeInDanger(Player ai, boolean serious, int payment) {
        Player opponent = ComputerUtilNN.getOpponentFor(ai);
